...
## Неделя 13 (QA и деплой)

- **Unit-тесты:** `pytest` из корня проекта (конфиг `pytest.ini`, тесты в `tests/` — правила `check_rules` / `load_rules`, дымовые сценарии чата).
- **Зависимости:** `requirements.txt` (пины из `Pipfile.lock`), для тестов — `requirements-dev.txt`.
- **Чистая машина:** пошагово в [docs/неделя13-чистая-машина.md](docs/неделя13-чистая-машина.md).